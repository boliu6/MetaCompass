                      ...
                      Sint(*apply)(void *,Uint,Uchar *,Uint), int pnum, int nump)
{
  ...
  int numseq = multiseq->numofsequences;
  int iindex = 0;
  while(1)
  {
    ...
    {
      start = seq + multiseq->markpos.spaceUint[iindex-1] + 1;
    }
    if(iindex == multiseq->numofsequences - 1)
    {
      end = seq + multiseq->totallength;
    } else
    {
      end = seq + multiseq->markpos.spaceUint[iindex];
    }
    ...
    // apply is findmaxmatchesonbothstrands;
    // applyinfo is matchprocessinfo
    if(apply(applyinfo,iindex,start,(Uint) (end - start)) != 0)
    {
      return -1;
    }
  }

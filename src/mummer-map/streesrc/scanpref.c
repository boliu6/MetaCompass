...
/*
  This function implements the function scanprefix, restricted to the case that
  the starting location is a node location. stree is the suffix
  tree, loc is the resulting location, btptr is the branching node.
  The string s to be scanned is defined by left and right.
  The function returns NULL, if s is scanned completely.
  Otherwise, it points to a suffix of s. 
  rescanlength is the length of a prefix of 
  s that already occurs in the suffix tree. It is always safe
  to set rescanlength to 0.
*/
